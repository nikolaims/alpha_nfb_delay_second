...
class FixSPoC(SPoC):
    def fit(self, X, y):
        ...
        Cz = np.sum([y_*X_.dot(X_.T) for y_, X_ in zip(y, X)], 0)
        C = np.sum([np.abs(y_)*X_.dot(X_.T) for  y_, X_ in zip(y[:n_epochs//2], X[:n_epochs//2])], 0) + np.eye(n_channels)*self.reg
...
for subj_id in range(40):
    ...
    if LINEAR_TREND: spoc_y = (np.arange(len(spoc_y))-len(spoc_y)//2)
    ...
    # cov = [y*X.T.dot(X) for y, X in zip(spoc_y, spoc_X)]
    # linalg.eigh(cov)
    ...
    for n_comp in [10]:
        ...
        for k in range(n_comp):
            ...
            plot_topomap(topo[:, k], pos, axes=ax, show=False)
