...
stats_df = pd.DataFrame(columns=['subj_id', 'fb_type', 'P4', 'P4_SSD', 'P4_SSD_CSP'])
for subj_id in range(1, 2):
    ...
    X_alpha_clear = np.array([ssd_patterns[:, ssd_evals>1].dot(csp_patterns[:, csp_evals>1.].dot(x[csp_evals>1.])) for x in X_alpha_csp])
    X_alpha_ssd = np.array([ssd_patterns[:, ssd_evals>1].dot(x) for x in X_alpha])
    ...
    p4 = X_band[:, CHANNELS.index('P4')].var(1)
    p4_gain = p4[segment_number>=n_epochs//2].mean()/p4[segment_number<n_epochs//2].mean()
    p4_ssdcsp = X_alpha_clear[:, CHANNELS.index('P4')].var(1)
    p4_ssdcsp_gain = p4_ssdcsp[segment_number >= n_epochs // 2].mean() / p4_ssdcsp[segment_number < n_epochs // 2].mean()
    p4_ssd = X_alpha_ssd[:, CHANNELS.index('P4')].var(1)
    p4_ssd_gain = p4_ssd[segment_number >= n_epochs // 2].mean() / p4_ssd[segment_number < n_epochs // 2].mean()
    stats_df = stats_df.append({'subj_id': subj_id, 'fb_type': fb_type, 'P4': p4_gain, 'P4_SSD': p4_ssd_gain, 'P4_SSD_CSP': p4_ssdcsp_gain}, ignore_index=True)
    print(stats_df.tail())
    ...
    main_ax.plot(p4, label='P4 {:.2f}'.format(p4[segment_number>=n_epochs//2].mean()/p4[segment_number<n_epochs//2].mean()))

    main_ax.plot(p4_ssdcsp, label='P4 ssd csp {:.2f}'.format(p4_ssdcsp[segment_number>=n_epochs//2].mean()/p4_ssdcsp[segment_number<n_epochs//2].mean()))
...
for ax, name in zip(axes, ['P4', 'P4_SSD', 'P4_SSD_CSP']):
    sns.boxplot('fb_type', name, order=['FB0', 'FB250', 'FB500', 'FBMock'], data=stats_df, ax=ax)
    sns.swarmplot('fb_type', name, order=['FB0', 'FB250', 'FB500', 'FBMock'], data=stats_df, color='k', ax=ax)
    ax.set_title(name)
    ax.set_ylim(0.8, 1.8)
...
name = 'P4_SSD_CSP'
g1 = stats_df.query('fb_type=="FB250"')[name].values
g2 = stats_df.query('fb_type=="FB500"')[name].values
print(ranksums(g1, g2))
