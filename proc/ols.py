...
for fb_type, fb_type_df in all_stats_df.groupby('fb_type'):
    for s, (subj_id, subj_df) in enumerate(fb_type_df.groupby('subj_id')):
        for c, (ch, ch_df) in enumerate(subj_df.groupby('channel')):
            ...
            y_df = y_df.append(pd.DataFrame({'fb_type': fb_type, 'subj_id': 's'+str(subj_id), 'channel': ch, 'k': np.linspace(0, 1, 30), 'env': curve-np.min(curve)*0.0}))
...
for k, fb_type in enumerate(['FB0', 'FB250', 'FB500', 'FBMock']):
    results = smf.ols('np.log(env) ~ k:fb_type + C(subj_id) + C(channel) - 1', data=y_df.query('fb_type == "{}"'.format(fb_type))).fit()
    ...
    plot_topomap(np.exp(np.concatenate([[0], results.params[10:10+31]])), MONTAGE.get_pos()[indx], vmin=0, vmax=1, axes=ax[k], show=False, cmap='Reds', contours=[0.75, 1])


    for s, (subj_id, subj_df) in enumerate(y_df.query('fb_type == "{}" & channel=="C3"'.format(fb_type)).groupby('subj_id')):
        p4ax[k].plot(np.log(subj_df['env'].values)-results.params[s] - 0*results.params[[('P4' in s) for s in results.params.index]][0], alpha=0.9, label=subj_id)

    p4ax[k].set_title(fb_type)
