...
for m, metric_type in enumerate(['magnitude', 'n_spindles', 'amplitude', 'duration']):
    for k, fb_type in enumerate(['FB0', 'FB250', 'FB500', 'FBMock']):
        ...
        im = plot_topomap(np.exp(results.params[:32]-max(results.params[:32]))*(np.exp(results.params[-1])), MONTAGE.get_pos()[indx], vmin=0, vmax=1.2, axes=ax[m, k], show=False, cmap='inferno', contours=[1.25])
