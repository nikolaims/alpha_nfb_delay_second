...
for factors_values, group in scores_df.groupby(factor_names):
    for pvalue_type in ['FBMock', 'FB500', 'FB250', 'FB0']:
        ...
        ranksum = ranksums(group['score'].values, mock['score'].values)
        ...
        pvalue_dict = dict(zip(factor_names + ['runksum', 'comparison', 'pvalue'], list(factors_values) + [[ranksum], [pvalue_type], [pvalue]]))
